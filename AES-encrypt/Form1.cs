...
/* f */     {0x17,0x2b,0x04,0x7e,0xba,0x77,0xd6,0x26,0xe1,0x69,0x14,0x63,0x55,0x21,0x0c,0x7d},
        ...
        private byte[] rcon = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,
        ...
        private void mode128 ()
        {
            ...
            if(do_dai_khoa != 16)
            {
                MessageBox.Show("Sai độ dài khoá, yêu cầu bao gồm 16 ký tự (128bit)");
            }
            else
            {
                KeyExpantion(4, 10);
        ...
        private void mode192()
        {
            ...
            if (do_dai_khoa != 24)
            {
                MessageBox.Show("Sai độ dài khoá, yêu cầu bao gồm 24 ký tự (192bit)");
            }
            else
            {
                KeyExpantion(6, 12);
        ...
        private void mode256()
        {
            ...
            if (do_dai_khoa != 32)
            {
                MessageBox.Show("Sai độ dài khoá, yêu cầu bao gồm 32 ký tự (256bit)");
            }
            else
            {
                KeyExpantion(8, 14);
        ...
        private void KeyExpantion(int Nk, int Nr)
        {
            ...
            while(i < Nk)
            {
                expanKey[0, i] = Convert.ToByte(tbKhoa.Text[i * 4]);
                expanKey[1, i] = Convert.ToByte(tbKhoa.Text[i * 4 + 1]);
                expanKey[2, i] = Convert.ToByte(tbKhoa.Text[i * 4 + 2]);
                expanKey[3, i] = Convert.ToByte(tbKhoa.Text[i * 4 + 3]);
                i++;
            }
            while(i < 4 * (Nr + 1))
            {
                ...
                {
                    temp = RotWords(temp);
                    temp[0] = (byte)(SubByte(temp[0]) ^ rcon[(i / Nk) - 1]);
